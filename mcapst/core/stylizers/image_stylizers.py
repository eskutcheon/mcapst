...
from .stylizer_builder import prep_single_image
...
class BaseImageStylizer(BaseStylizer):
    #! REMOVE supported_args usage next - compatibility with the transform call signature can be ensured by validators later
    ...
    def stylize_from_images(
        ...
        with torch.no_grad(): # forward inference of self.revnet acts as the feature encoder
            content_img = prep_single_image(content_img, self.max_size, downscale=self.revnet.down_scale).clamp(0,1)
    ...
    def transform(
        ...
        style_paths: torch.Tensor, #Union[str, List[str], torch.Tensor, List[torch.Tensor]],
        ...
        if isinstance(style_paths, torch.Tensor) and style_paths.device != content_img.device:
            style_paths = style_paths.to(device=content_img.device)
...
"""
    ? NOTE on use_segmentation:
        ...
        - [ ] could potentially move the label filtering in cWCT.get_masked_target_features to an earlier preprocessing step
